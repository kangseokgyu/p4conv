...
class ConversionState:
    ...
    def file_payload(self, files: List[Path]) -> List[Dict[str, Any]]:
        ...
        for index, path in enumerate(files):
            ...
            ftype = probe_file_type(path)
            if ftype == "live_photo":
                already_converted = (Path.cwd() / path.with_suffix(".jpg").name).exists() and (Path.cwd() / path.with_suffix(".mov").name).exists()
            else:
                already_converted = (Path.cwd() / path.with_suffix(".mov").name).exists()
...
function renderFiles(){
  ...
  const liveCount=files.filter(f=>f.file_type==='live_photo').length;
  $('count-all').textContent=files.length;
  $('count-video').textContent=videoCount;
  $('count-live').textContent=liveCount;

  const filtered=files.filter(f=>currentFilter==='all'||f.file_type===currentFilter);
  $('files').innerHTML=filtered.map(f=>{
    const badge=f.file_type==='live_photo'?'<span class="type-badge live-photo">📸 Live Photo</span>':'<span class="type-badge video">🎬 동영상</span>';
    return `<tr><td><input type="checkbox" value="${f.id}" ${f.alreadyConverted?'disabled':''}></td><td>${badge}</td><td><div class="file-name">${esc(f.name)}</div></td><td>${f.modified}</td><td>${f.duration}</td><td>${f.size}</td><td>${progress(f)}</td></tr>`;
  }).join('');
